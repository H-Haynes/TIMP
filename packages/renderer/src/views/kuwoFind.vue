...
<script lang="ts" setup>
    ...
    import {getCategoryListKW,getRecommendKW,getAlbumListKW,getBannerKW, getRankListKW} from '/@/api/kuwo';
    ...
    watchEffect(()=>{
        loading.value = true;
        getRecommendKW().then((res: any)=>{
            if(res.data.code === 200) {
              const list = res.data.data.list.map(ele=>({
                name:ele.name,
                picUrl:ele.img,
                id:ele.id,
              }));
    ...
    onMounted( async()=>{
      ...
      const result = await getCategoryListKW();
      if(result.data.code === 200){
        let list = result.data.data.reduce((prev,cur) => {
    ...
    const getAlbumList = async (category:number,pageNum=1) => {
      ...
      try{
        const result = await getAlbumListKW(category,page.value);
        if(result.data.code === 200){
          console.log(result);
          const list = result.data.data.data.map(ele=>({
                name:ele.name,
                picUrl:ele.img,
                id:ele.id,
          }));
    ...
    const getRankAlbum = async () =>{
      ...
      const result = await getRankListKW();
      if(result.data.code === 200){
        const list = result.data.data.reduce((prev,cur)=>{
